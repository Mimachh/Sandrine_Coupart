        ...
        <div class="w-full">
                ...
                <form class="bg-white rounded-lg px-4 py-5 mt-2 shadow hover:shadow-xl">
                        ...
                        <div class="mb-3 text-center space-y-2">
                            <x-jet-label class="underline font-semibold" for="name">Nom de l'allergene</x-jet-label>
            ...
            <h3 class="text-green-600 text-lg text-center mb-5"> {{ $allergenes->count() }} Allergene(s) répertorié(s)</h3>
                ...
                <table class="divide-y divide-gray-300">
                    <thead class="bg-gray-200">
                        <tr class="px-6 py-2 text-xs text-gray-500 divide-x divide-gray-300">
                            ...
                            <th class="px-6 py-4"scope="col">Nom de l'allergène</th>
                            <th class="px-6 py-4" scope="col">Liste des patients allergiques</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-300">
                        @foreach ($allergenesWithPagination as $allergene)
                            <tr class="whitespace-nowrap text-xs text-center text-gray-500 divide-x">
                                <th class="bg-gray-200" scope="row">{{ $allergene->id }}</th>
                                <td>{{ $allergene->name}}</td>
                                <td>
                                    <ul>
                                        @foreach($allergene->users as $patient)
                                            <li>
                                ...
                                <td>
                                    <ul>
                                        @foreach($allergene->recettes as $recette)
                                            <li>
                                                {{ $recette->title }}
                                            </li>
                                        @endforeach
                                ...
                                <td class="px-6 py-4">
                                    <button wire:click.prevent="edit({{ $allergene->id }})" type="button" class="px-4 py-1 text-sm text-blue-600 bg-blue-200 rounded-full">Modifier</button>
                                    <button wire:click.prevent="delete({{ $allergene->id }})" type="button" class="px-4 py-1 text-sm text-red-400 bg-red-200 rounded-full">Supprimer</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
